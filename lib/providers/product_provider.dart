...
class ProductProvider with ChangeNotifier {
  ...
  List<ProductModel> findByCategoryName({required String ctgName}) {
    ...
    try {
      await productDB
        ...
        for (var element in productSnapshot.docs) {
          _products.insert(0, ProductModel.fromFirebase(element));
        }
      });
      notifyListeners();
      return _products;
    ...
    try {
      return productDB.snapshots().map((snapshot) {
        ...
        for (var doc in snapshot.docs) {
          _products.insert(0, ProductModel.fromFirebase(doc));
        }
        return _products;
      });
    } catch (e) {
